...
4. **Parallel Execution** – `/api/research/:id/run` (`scheduleResearchRun`) transitions the doc to `running`, then kicks off OpenAI Deep Research and Gemini in parallel using `Promise.all`. Each provider sets its own `status`, `startedAt`, `completedAt`, `durationMs`, `result`, and `error` as it settles so partial success is recorded. The research status flips to `completed` when at least one provider succeeds; otherwise it becomes `failed`. Deep Research invocations flow through an in-memory token bucket (configurable via `OPENAI_DR_RUNS_PER_MINUTE`) and a Firestore-backed distributed limiter so horizontal scaling cannot blow past OpenAI’s 200 k TPM ceiling. Runs cap their own output via `OPENAI_DR_MAX_OUTPUT_TOKENS` and trim refined prompts beyond `OPENAI_DR_MAX_PROMPT_CHARS` with a warning; polling continues until the job resolves, with optional upper bounds exposed via `OPENAI_DR_POLL_MAX_ATTEMPTS` / `OPENAI_DR_POLL_MAX_DELAY_MS` for ops tuning.
   Raw provider payloads are sanitized (undefined keys dropped, sparse array entries removed) before persisting so Firestore never rejects the document on deeply nested `undefined` fields.
   Targeted retries hit `/api/research/:id/providers/:provider/retry`, which marks the chosen provider `running`, leaves the peer state untouched, and reuses the same limiter/finalization pipeline once the run settles.
5. **Finalization** – `/api/research/:id/finalize` builds the comparative PDF via `pdf-lib`, persists it to Firebase Storage when `FIREBASE_STORAGE_BUCKET` is available (otherwise exposes an in-memory `buffer://` path), records `report.pdfPath`, then invokes `sendResearchReportEmail`. The email orchestrator refreshes Gmail OAuth tokens when possible, falls back to SendGrid on authentication/network failures, and stamps `report.emailStatus`, `report.emailedTo`, and `report.emailError` accordingly so the UI can surface delivery banners immediately. The run orchestrator now invokes this finalize pipeline automatically whenever a session completes successfully, fetching the owner’s email from the user repository so report delivery happens without extra UI steps (the API route remains available for manual retries). If a stored profile email is missing, the pipeline reuses the session email supplied with the run request or, in demo mode, generates a deterministic `demo-user+<uid>@example.com` fallback recipient so delivery never blocks the flow. Provider responses authored in Markdown are pre-processed so headings, lists, quotes, and fenced code render with the appropriate Helvetica Oblique / Courier fonts instead of raw syntax.
   Each page now renders within a subtle border frame and the text wrapper force-breaks long tokens (e.g., URLs) so content never overflows the printable margins. Provider sections present an “Executive Summary” followed by a “Findings” block (with “Key Insights” and “Primary Sources” subheadings), and source URLs are written as clickable link annotations in the PDF.
...
The research detail view exposes explicit retry controls: failed provider executions surface a “Retry run” button, provider cards expose dedicated “Retry OpenAI Deep Research” / “Retry Google Gemini” actions once an attempt has completed, email delivery failures expose “Retry email delivery”, and the creation form provides a retry affordance alongside error messaging so demo operators can recover without reloading.
...
- `/sign-in` uses `signInWithPopup` with the Firebase Google provider, enforcing `browserLocalPersistence` and redirecting to the `redirectedFrom` query param or `/dashboard` (also used when redirecting from `/`) after success. The server evaluates the `firebaseToken` cookie (or local dev bypass) and immediately redirects authenticated visitors so the client never flashes the sign-in screen.
...
- Integrate OpenAI Deep Research & Gemini using provider utilities. Deep Research requests now respect `OPENAI_PROJECT_ID`
  (when present) by supplying the `OpenAI-Project` header while calling the Responses API. Defaults for clarification,
  prompt rewriting, and execution models can be overridden via `OPENAI_CLARIFIER_MODEL`, `OPENAI_PROMPT_WRITER_MODEL`, and
  `OPENAI_DR_MODEL` respectively.
